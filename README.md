# Sisit: Inventory Management System

Manual product tracking is time consuming and prone to human error, making it difficult to keep up with the demands of a fast paced market. The Inventory Management System for SISIT Bag Retail Brand aims to aid these challenges, innovation of the inventory processes through a Python based application that is developed with the PyQt6 library.

## Project Objective:
By providing a centralized and user-friendly platform, it reduces repetitive tasks, minimizes human error, and ensures data accuracy. Additionally, its built-in search and sorting functions make it easier for users to manage large inventories effectively.

## Key Features:
  - Role based log-in System
  - Search and Filter function
  - Add to Cart & Check-out System
  - Customer Orders Management
  - Consumers’ Feedback

## Libraries
  - PyQt6
  - dotenv<br/>
### Built-in python libraries
  - os, sys, pathlib
  - json
  - datetime
  - subprocess

## Developed by
Lance Chester V. Bolito
- Document Organizer<br/>

Gabriel A. Cabrera
- Developer<br/>

Ranzel Aldous Gabriel L. Dueñas
- Developer<br/>

Ronniel B. Cardenas
- Visual Designer<br/>

Ace Marlou R. Tadalan
- Conceptual Designer<br/>

The project demonstrated an effective application of Object-Oriented Programming (OOP) resulting in a maintainable system architecture. The successful implementation of these functionalities reflects not only technical proficiency, yet it is also a practical understanding of how both of users may be centered by a software design to enhance their productivity and business growth.
